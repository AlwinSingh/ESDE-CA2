...
const uploadFile = function uploadFile(file, callback) {
    console.log(file);

    // upload image here
    // --> XXE Solution
    // Changed allowed formats to png only to prevent XXE.
    cloudinary.uploader.upload(file.path, { allowed_formats: ['png'], upload_preset: 'upload_to_design' })
        .then((result) => {
...
const createFileData = function createFileData(imageURL, publicId, userId, designTitle, designDescription) {
...
const getFileData = function getFileData(userId, pageNumber, search) {
    ...
    var queryData = [userId, limit, offset, userId];
    ...
    if ((search == '') || (search == null)) {
        ...
        search = '%' + search + '%'
        console.log(search);
        queryData = [userId, search, limit, offset, userId, search];
    }
    //--------------------------------------------------------------------
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(designFileDataQuery, queryData, (err, results) => {
                    if (err) {
                        ...
                        for (let i = 0; i < results[0].length; i++) {
                            results[0][i].cloudinary_url = decrypt(results[0][i].cloudinary_url);
                        }
...
module.exports.createFileData = createFileData;
